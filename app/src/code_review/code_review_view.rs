...
pub struct FileState {
    ...
    /// Stored content for deferred editor construction. When a file starts
    /// collapsed, we skip the expensive `LocalCodeEditorView` /
    /// `CodeEditorView` / `CommentEditor` construction and keep only the
    /// lightweight text content. The editor is created lazily when the user
    /// expands the file.
...
impl CodeReviewView {
    ...
    fn build_view_state_for_file_diffs(
        ...
        for file in files {
            ...
            // Defer editor construction for collapsed files to avoid multi-GB
            // memory spikes when many files are present (APP-4518). The editor
            // is created lazily via `ensure_editor_for_file` when the user
            // expands the file.
            let (editor_state, deferred_content_at_head) = if is_expanded {
                ...
                // Store content_at_head for deferred construction.
    ...
    /// Lazily creates the editor for a file that was deferred during
    /// `build_view_state_for_file_diffs` (collapsed files skip editor
    /// construction to avoid multi-GB memory spikes). Called when the user
    /// expands a collapsed file via `ToggleFileExpanded` or `FileSelected`.
    fn ensure_editor_for_file(&mut self, file_path: &str, ctx: &mut ViewContext<Self>) {
        ...
        // Editor already exists — nothing to do.
        if file_state.editor_state.is_some() {
            return;
        }

        // Take the deferred content to build a synthetic FileDiffAndContent.
        ...
        // Re-borrow after the editor construction calls above.
        let Some(repo) = self.active_repo.as_mut() else {
            return;
        };
        let CodeReviewViewState::Loaded(loaded_state) = &mut repo.state else {
    ...
    fn handle_action(&mut self, action: &CodeReviewAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeReviewAction::ToggleFileExpanded(path) => {
                ...
                // Lazily create the editor if this file was deferred (APP-4518).
                if now_expanded {
