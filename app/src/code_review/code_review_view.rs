...
pub struct FileState {
    ...
    /// Stored for deferred editor creation when a collapsed file is later expanded.
    /// This avoids eagerly creating expensive editor views for files that start collapsed
    /// (e.g. large diffs, autogenerated files), which can consume several GB of memory.
...
impl CodeReviewView {
    ...
    fn build_view_state_for_file_diffs(
        ...
        for file in files {
            ...
            // Defer editor creation for collapsed files to avoid loading full file
            // buffers, computing styled blocks, and creating UI views upfront. For
            // large diffs this can save several GB of memory.
            let (editor_state, content_at_head) = if is_expanded {
                ...
                // Store content_at_head for deferred editor creation when the file is later expanded.
    ...
    /// Lazily creates an editor for a file at the given index that was initially collapsed.
    /// Called when a user expands a file that had deferred editor creation
    /// (see `build_view_state_for_file_diffs`).
    fn ensure_editor_for_file_at_index(
        &mut self,
        file_index: usize,
        ctx: &mut ViewContext<Self>,
    ) {
        // Extract the data needed for editor creation from the file state.
        // We need to do this in a separate scope to avoid borrow conflicts.
        let file_data = {
            let Some(repo) = self.active_repo.as_mut() else {
                return;
            };
            ...
            let Some((_, file_state)) = state.file_states.get_index_mut(file_index) else {
                return;
            };
            if file_state.editor_state.is_some() {
                return;
            }
            // Take the data we need — file_diff is cloned, content_at_head is moved out.
            Some(FileDiffAndContent {
                file_diff: file_state.file_diff.clone(),
                content_at_head: file_state.content_at_head.take(),
            })
        };

        let Some(file_diff_and_content) = file_data else {
            return;
        };

        // Now create the editor with an immutable borrow on self.
        let editor_state = {
            #[cfg(not(target_family = "wasm"))]
            {
                if self.repo_path().is_some() {
                    self.create_code_review_model_with_global_buffer(
                        &file_diff_and_content,
                        ctx,
                    )
                } else {
        ...
        // Assign the editor state back to the file state.
        if let Some(repo) = self.active_repo.as_mut() {
            if let CodeReviewViewState::Loaded(state) = &mut repo.state {
                if let Some((_, file_state)) = state.file_states.get_index_mut(file_index) {
    ...
    fn handle_action(&mut self, action: &CodeReviewAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeReviewAction::ToggleFileExpanded(path) => {
                ...
                // Lazily create the editor if expanding a file that had deferred creation.
                // We extract the data needed for editor creation first, then drop the mutable
                // borrow before calling into methods that take &self.
                if now_expanded {
                    let needs_editor = self
                        ...
                        .and_then(|repo| match &repo.state {
                            CodeReviewViewState::Loaded(state) => {
                                state.file_states.get_index(file_index)
                            }
                            _ => None,
                        })
                        .is_some_and(|(_, fs)| fs.editor_state.is_none());

                    if needs_editor {
                        self.ensure_editor_for_file_at_index(file_index, ctx);
                    }
                }
            ...
            CodeReviewAction::FileSelected(file_index) => {
                ...
                // Lazily create the editor if selecting a file that had deferred creation.
                if !was_expanded {
                    let needs_editor = self
                        .active_repo
                        .as_ref()
