...
struct CloudAgentComputerUseWidget {
    ...
    artifact_storage_settings_link_index: HighlightedHyperlink,
}
impl CloudAgentComputerUseWidget {
    fn render_artifact_storage_description(
        &self,
        is_enabled: bool,
        appearance: &Appearance,
        app: &AppContext,
        ...
        let mut fragments = vec![FormattedTextFragment::plain_text(
            "Store artifacts generated by Computer Use, including screenshots attached to PRs.",
        )];

        let workspaces = UserWorkspaces::as_ref(app);
        let team = workspaces.current_team().or_else(|| {
            workspaces
                .workspaces()
                .iter()
        ...
        let (settings_label, settings_url) = team
            .map(|team| {
                (
                    "Agent Data Storage",
                    AdminActions::admin_platform_link_for_team(team.uid),
                )
            })
            .unwrap_or_else(|| ("Admin Panel", AdminActions::admin_panel_link()));

        fragments.extend([
            FormattedTextFragment::plain_text(
                " Team admins can configure self-hosted artifact storage in ",
            ),
            FormattedTextFragment::hyperlink(settings_label, settings_url),
            FormattedTextFragment::plain_text("."),
        ]);
        ...
        Container::new(
            FormattedTextElement::new(
                ...
                appearance.ui_font_family(),
                appearance.ui_font_family(),
                styles::description_font_color(is_enabled, app).into(),
                self.artifact_storage_settings_link_index.clone(),
            )
            .with_hyperlink_font_color(appearance.theme().accent().into_solid())
            .register_default_click_handlers(|url, ctx, _| {
                ctx.dispatch_typed_action(AISettingsPageAction::HyperlinkClick(url));
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn search_terms(&self) -> &str {
        "oz cloud agent computer use orchestration multi-agent artifacts screenshots self-hosted agent data storage"
    }
    ...
    fn render(
        ...
        Flex::column()
            ...
            .with_child(self.render_artifact_storage_description(
                is_computer_use_effectively_enabled,
                appearance,
                app,
            ))
